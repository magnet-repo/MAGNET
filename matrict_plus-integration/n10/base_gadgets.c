 ...
 */
{
	...
	for(i=0; i < N_SHARES; ++i)
		for(j=i+1; j < N_SHARES; ++j){
			z_ij  = ((__int128_t)rand_uint32_() << 96) | ((__int128_t)rand_uint32_() << 64) | ((__int128_t)rand_uint32_() << 32) | rand_uint32_();
			z_ji  = (x[i] & y[j]) ^ z_ij;
			z_ji ^= (x[j] & y[i]);
			r[i] ^= z_ij;
			r[j] ^= z_ji;
 ...
 */
{
	...
	for(i=0; i < N_SHARES; ++i)
		for(j=i+1; j < N_SHARES; ++j){
			r = ((__int128_t)rand_uint32_() << 96) | ((__int128_t)rand_uint32_() << 64) | ((__int128_t)rand_uint32_() << 32) | rand_uint32_();
			res[i] ^= r;
			res[j] ^= r;
		}
}
