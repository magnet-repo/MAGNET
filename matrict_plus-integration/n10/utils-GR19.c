...
int get_CDT_SIZE() {
    ...
    const long double inv = 1.0L / (SIG_ * sqrtl(2.0L * PI));

    long double cdf = inv;
    int n = 2;

    for (int i = 1; ; ++i) {
        long double w = inv * expl(-(long double)(i * i) / (2.0L * SIG_ * SIG_));
        if (w * SCALE < 1.0L) {
        	++n;
        	break;
        }
...
double calc_median_(double values[ITER_]) {
    qsort(values, ITER_, sizeof(values[0]), cmp_double_);

    double mid1 = values[ITER_/2 - 1];
    double mid2 = values[ITER_/2];
...
void print_shares_bits_vs(int* x, int N_){
    int i;
    int val=0;
    for(i=0; i < N_-1; ++i){
        print_bits(x[i]); printf(" | ");
